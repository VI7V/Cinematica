...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (result?.ok) {
        toast.success("Welcome to Cinematica!");
        router.push("/");
      }
    } catch {
    ...
    <div className="min-h-screen bg-cinema-black flex items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="bg-cinema-charcoal border border-cinema-gold/20 rounded-2xl p-8 shadow-card">
          <h2 className="font-cinzel text-cinema-cream text-xl font-semibold mb-1">Create Account</h2>
          <p className="text-cinema-muted text-sm mb-6">Start building your cinema vault</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="relative">
                <FiUser className="absolute left-3 top-1/2 -translate-y-1/2 text-cinema-muted text-sm" />
                <input
                  type="text"
                  value={name}
            ...
            >
              {loading ? "Creating account..." : "Open the Vault"}
            </button>
          </form>
          ...
          <p className="text-center text-cinema-muted text-sm">
            Already a member?{" "}
            <Link href="/auth/login" className="text-cinema-gold hover:text-cinema-gold-light transition-colors">
              Sign in
            </Link>
          </p>
        </div>
